...
-- ROLLBACK 을 수행해서 트랜잭션에 저장된 DML 다 지우기
ROLLBACK;
...
-- 참조 관계에 있다면 부모 테이블은 삭제 불가!


-- 해결 방법1) 참조하는 자식 테이블(TB2) 를 먼저 삭제하거나 FK 제약조건 삭제

-- 해결 방법2) CASCADE CONSTAINTS 옵션 사용하기
...
SELECT * FROM DEPT_COPY;
-- 1) 컬럼명 변경 : ALTER TABLE 테이블명 
--									RENAME COLUMN 컬럼명 TO 변경명;
-- DEPT_TITLE -> DEPT_NAME
ALTER TABLE DEPT_COPY
RENAME COLUMN DEPT_TITLE TO DEPT_NAME;

-- 2) 제약조건명 변경 : ALTER TABLE 테이블명 
