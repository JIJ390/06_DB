...
/* TEST_USER 계정 접속(localhost) */
-- SELECT 권한 부여 받은 KH 계정의 EMPLOYEE 테이블 조회

-- KH_JIJ 이 가진 EMPLOYEE 테이블
SELECT * FROM KH_JIJ.EMPLOYEE;

SELECT * FROM KH_JIJ.DEPARTMENT;
...
FROM TEST_USER;
-- 권한을 부여할 때는 TO / 권한을 회수할 때는 FROM


/* TEST_USER 계정 접속(localhost) SELECT 확인*/
