...
-- HAVING 절에서만 그룹 함수를 조건에 사용 가능 
-- SUM(SALARY) 를 사용할 수 없음
...
/* GROUP BY 에 작성된 컬럼명만 SELECT 절에 작성할 수 있다!!! */
...
WHERE SALARY IN (
	...
	FROM EMPLOYEE
	GROUP BY DEPT_CODE);
-- 다른 부서에 같은 급여를 받는 사원이 존재할 가능성 있음

-- 사수에 해당하는 직원에 대해 조회 
--  사번, 이름, 부서명, 직급명, 구분(사수 / 직원)

-- 1) 사수에 해당하는 사원 번호 조회
--> MANAGER_ID 컬럼에 작성된 사번을 가진 사원 == 사수
--  DISTINCT : 컬럼값 중복 제거
SELECT DISTINCT MANAGER_ID
FROM EMPLOYEE
WHERE MANAGER_ID IS NOT NULL; -- 7 행 조회
...
WHERE JOB_NAME = '과장'
	AND SALARY > ALL (
		SELECT SALARY		-- 	3800000, 3480000, 3490000, 2550000
		FROM EMPLOYEE
		JOIN JOB USING(JOB_CODE)
		WHERE JOB_NAME = '차장');
...
WHERE (DEPT_CODE, JOB_CODE) = (
		...
		AND SUBSTR(EMP_NO, 8, 1) IN ('2', '4'));
--> 여러 컬럼을 뭈어서 한 번에 비교해
-- 모두 같은 행만 조회결과(RESULT SET) 에 포함  시킴
...
-- 보통의 SELECT 문은 서브 쿼리 -> 메인코리 순서로 해석하는데
-- 상관 서브쿼리는
...
WHERE EXISTS ( -- 서브쿼리 조회 결과가 있으면 TRUE == 해당 행을 결과에 포함
	SELECT '사수 존재'		-- 존재하는지만 확인하기 때문에 아무거나 넣어도 무관
	
-- 서브쿼리에서 조회되는 컬럼 값은 중요한게 아님!!
...
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE MANAGER_ID IS NOT NULL;
-- 이 경우 MANAGER_ID 가 100으로 존재하지 않는 사번인 이태림이 포함됨
