...
WHERE BONUS IS NOT NULL			-- 2. 어떤 행만 조회할 지 지정
...
-- 부서별 급여 평균이 3500000원 이상인 부서를 조회하여 부서코드 오름차순으로 정렬
SELECT DEPT_CODE, AVG(SALARY)
FROM EMPLOYEE
WHERE SALARY >= 3500000
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE ASC;
--> 개인 급여가 3,500,000 이상인 사람들의 부서별 평균 (잘못된 SQL)
...
GROUP BY JOB_CODE
ORDER BY JOB_CODE ASC;	-- 4 행 조회 (0 명은 조회 결과에 포함 X)


/* 통계적인 측면에서는 아래 방법이 더 효용있음 */
--> COUNT(컬럼명) : 해당 컬럼에 값이 있는 행이 몇 개인지 카운트
----> NULL 이면 카운트 에서 제외
...
-- 부서 코드 오름차순으로 정렬하세요
SELECT DEPT_CODE, FLOOR(AVG(SALARY)) "급여 평균", COUNT(*)
FROM EMPLOYEE
WHERE EMP_NO LIKE '8%'
--WHERE EMP_NO LIKE '8_____________'
...
-- 앞 쪽에 작성된 SQL 문에서 뒤쪽 조건 빼기
-- 부서 코드 D5 중 급여가 400만 초과인 직원 제외

--> 교집합인 심봉선, 대북혼을 제외한
--	앞쪽 SELECT의 RESULT SET 이 조회됨
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'	
