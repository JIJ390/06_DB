...
BEGIN 
	FOR I IN 1..2000 LOOP
		...
		VALUES(
			...
			1, 
			CEIL(DBMS_RANDOM.VALUE(0, 3))				-- 0 이상 3 미만 올림 처리	// 보드 코드
		);
	END LOOP;
END;
...
SELECT 
	...
	RECEIVE_MEMBER_NO,
	NOTIFICATION_CHECK,
	CASE 	-- 시 분 초 부분이 0으로 바뀜 / 절삭, 오늘 온 알람이면 시분초를 보여줌
...
/* NOTIFICATION 테이블 NOTIFICATION_CHECK CHECK 제약 조건 추가 */
...
-- 검색 순서 -1 => 뒤에서부터 검색
SELECT 
	SUBSTR(PROFILE_IMG, INSTR(PROFILE_IMG, '/', -1) + 1) AS "FILE_NAME"
FROM "MEMBER"
