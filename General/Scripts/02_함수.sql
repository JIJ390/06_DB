...
FROM DUAL; 	-- 3 !! (INDEX 아님, 1 부터 시작하는 COUNT 개념 (0 이 아닌 1 부터 시작!!))
...
SELECT 
	123.456,
	ROUND(123.456),			-- 123
	ROUND(123.456, 0),	-- 123   	(0 이 ROUND 기본값)
	ROUND(123.456, 1),	-- 123.5 	(첫째자리에서 표현!, 둘째 자리 '5' 에서 반올림)
	ROUND(123.456, 2),	-- 123.46 (둘째자리에서 표현!, 셋째 자리 '6' 에서 반올림)
	ROUND(123.456, -1),	-- 120 		
	ROUND(123.456, -2)	-- 100		
FROM DUAL;
...
FROM DUAL;	-- 5.42 == 5 개월 13일

/* MONTHS_BETWEEN 이 지정된 두 날짜 사이의 차를 계산할 때
 * 훨씬 좋다!!(더 정확함)
 * -> 달 마다 길이(28, 29, 30, 31) 가 다 다르기 때문에
 * 	  직접 계산하면(30으로 나눔) 오차가 있을 수 있는데
 * 		MONTHS_BETWEEN 을 이용하면 이런 부분까지 모두
 * 		자동으로 적용되어 계산된다!
 * */
...
SELECT CURRENT_DATE,				-- 8  / 16
			 CURRENT_DATE + 31,		-- 9  / 16
			 CURRENT_DATE + 61,		-- 10 / 16
			 ADD_MONTHS(CURRENT_DATE, 1) "한 달 후",
			 ADD_MONTHS(CURRENT_DATE, 2) "두 달 후"
FROM DUAL;
...
-- 다음 달 1일, 다음 달 말일 조회하기
SELECT LAST_DAY(CURRENT_DATE) + 1 "다음 달 1일",
			 LAST_DAY(ADD_MONTHS(CURRENT_DATE, 1)) "다음 달 말일",
			 LAST_DAY(ADD_MONTHS(CURRENT_DATE, 1)) - (LAST_DAY(CURRENT_DATE))
FROM DUAL;
-- 9/1 00:00:00 ~ 9/30 23:59:59 + 1 이 남은 시간을 표시하기 때문에 없애줘야 함
...
--WHERE EXTRACT(YEAR FROM HIRE_DATE) LIKE '__1%'
...
-- 화폐 기호 + 자릿수 구분
SELECT 123456789, 
			 TO_CHAR(123456789, 'L999,999,999') 원,
			 TO_CHAR(123456789, '$999,999,999') 
FROM DUAL;

-- 모든 사원의 연봉을 
-- \OOO,OOO,OOO 으로 조회
SELECT TO_CHAR(SALARY * 12, 'L999,999,999')
FROM EMPLOYEE;
...
--> 년, 월, 일, 시, 분, 초 는 시간 관련 기호로 인식되지 않아서 오류가 발생

--> [해결방법] : "" 로 감싸서 패턴을 나타내는 기화 아닌 
--							 있는 그대로 출력하는 글자임을 명시
...
FROM DUAL;
-- TO_DATE 에 매개 변수가 한 개만 작성될 수 있는 경우
...
-- EMPLOYEE 테이블에서
-- 사번, 이름, 전화번호 조회
-- 단, 전화번호가 없다면(NULL) '없음' 으로 조회 (NULL VALUE 일 때)
SELECT 
	EMP_ID,
	EMP_NAME,
	NVL(PHONE, '없음') PHONE
...
-- RPAD(문자열, 길이, 바꿀 문자)
--     : 문자열 전체에서 "오른쪽"을 지정된 길이 만큼 다른 문자로 변경
--			(첫 문자열은 바꾸지 않을 부분, 길이는 문자열 전체 길이)
SELECT 
	EMP_ID,
	EMP_NAME,
	NVL2(PHONE,
...
AND SALARY BETWEEN 1000000 AND 4000000
ORDER BY SALARY ASC;
...
-- ELSE IF 와 유사 위 계산식을 빠져 나오면 범위 제외
...
-- 모든 사원의 급여 평균 조회
SELECT FLOOR(AVG(SALARY)) FROM EMPLOYEE; -- 4,091,140 (내림 처리)
...
--> TIP, SELECT 결과 집합인 RESULT SET 이
