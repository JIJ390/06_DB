...
/* 자료형
    ...
    ** CHAR / VARCHAR2 주의사항 **
    ...
    DATE : 날짜 타입
    TIMESTAMP : 날짜 타입 (MS 단위 + UTC 기준 시차)
 ...
 * 이름					: MEMBER_NAME /  VARCHAR2(15)		  (한글 5 자)
...
--> DEFAULT == 테이블 생성시 지정한 DEFUALT 값 : CURRENT_DATE
...
SELECT * FROM USER_USED_NN; -- 오류 안난  1 행만 삽입됨을 확인
...
--    제약조건종류(컬럼명) : 어떤 컬럼에 제약 조건을 설정할지 작성
...
INSERT INTO USER_USED_UK
VALUES(1, NULL, 'pass01', '나길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
--> ORA-00001: 무결성 제약 조건(KH_JIJ.USER_NAME_U)에 위배됩니다
...
-- ORA-00001: 무결성 제약 조건(KH_JIJ.USER_ID_NAME_U)에 위배됩니다
...
--> 기본키 중복으로 오류
--  ORA-00001: 무결성 제약 조건(KH_JIJ.USER_NO_PK)에 위배됩니다


INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
--  ORA-01400: NULL을 ("KH_JIJ"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
CREATE TABLE USER_USED_FK(
  ...
  GRADE_CODE NUMBER -- 해당 컬럼에 작성될 수 있는 값은 
  									-- USER_GRADE 테이블의 PK 컬럼값 (GRADE_CODE) 만 작성하게 제한
  									--> FK 제약조건 설정
...
-- ORA-02291: 무결성 제약조건(KH_JIJ.GRADE_CODE_FK1)이 위배되었습니다
-- - 부모 키가 없습니다
...
-- ORA-02292: 무결성 제약조건(KH_JIJ.GRADE_CODE_FK1)이 위배되었습니다
-- - 자식 레코드가 발견되었습니다

-- 자식 테이블인 USER_USED_FK 테이블에서
-- GRADE_CODE 컬럼의 값 10 을 참조하고 있기 때문에
-- 삭제 불가!!!
...
SELECT * FROM USER_USED_FK2;	-- 10 이 NULL 로 변했는지 확인
...
SELECT * FROM USER_USED_FK3;		-- 홍길동, 이순신 행 삭제
...
-- ORA-02290: 체크 제약조건(KH_JIJ.GENDER_CHECK)이 위배되었습니다
