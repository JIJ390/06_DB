...
AS SELECT * FROM EMPLOYEE;  -- 23 행 생성

CREATE TABLE EMP_M02
AS SELECT * FROM EMPLOYEE		-- 4 행 생성
   WHERE JOB_CODE = 'J4';
...
SELECT * FROM EMP_M01;			-- 23 행 
SELECT * FROM EMP_M02;			-- 5 행
		--> EMP_M02는 M01 테이블과 
		--  4 개의 행이 중복되고 1 개의 행이 다름 
...
-- 1 번 테이블 기준으로 병합 (2번 테이블 사용)
WHEN MATCHED THEN	-- 중복 확인용 KEY 가 매치되면 (일치하면)
UPDATE SET	-- 수정하겠다
...
WHEN NOT MATCHED THEN -- 증복되지 않으면/ 1번 테이블에 2번 테이블 행이 없으면!
...
SELECT * FROM EMP_M01; 
-- 급여가 0 원인 경우 병합된 것, 관두원은 새로 INSERT 됨
