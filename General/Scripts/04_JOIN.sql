...
FROM EMPLOYEE;
-- 직원번호, 직원면, 부서코드는 EMPLOYEE 테이블에 조회가능
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_ID, DEPT_TITLE
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); -- 21 행
-- 하동운, 이오리 제외

--> DEPT_CODE, DEPT_ID 가 일치하는 행 끼리 한 행으로 합쳐짐
...
-- INNER JOIN 이 JOIN 의 기본 값(생략 가능)
...
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- 23 행
...
-- DEPT_CODE 의 값이 
-- DEPT_ID 와 일치하지 않아도 결과에 포함(추가) 해라
...
-- DEPARTMENT 의 'D3', 'D4', 'D7' 도 결과에 포함
...
-- DEPARTMENT 의 'D3', 'D4', 'D7' 
-- 모두를 JOIN 결과에 포함시켜라
-- == LEFT UNION(합집합) RIGHT 와 같은 결과
...
--> 직접 사용 보단 NATURAL JOIN 의 실패 결과로 보여지는 모습이다!
--  라고 알아두는게 좋음
...
-- 반드시 두 테이블 간의 동일한 컬럼명, 타입을 가진 컬럼이 필요 (BYTE 수도 같아야 함!)
--> 없을 경우 교차조인이 됨.
...
-- EMPLOYEE, DEPARTMENT, LOCATION 테이블 조인하기
...
--JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
--JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE);
-- 앞부분이 눈덩이 굴리듯 거대한 집합이 되는것 연결 관계가 유연하다
...
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID) 	-- 부서가 없는 NULL 이 80년대생일 수 있어 LEFT 로 명시
WHERE 
	EMP_NO LIKE '8%'
